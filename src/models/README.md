...
| `regression` | `(B, D)` or `dict` | launch_speed, launch_angle, hit_distance_sc, spray_angle。MLP: `(B, D)` テンソル、MDN: `pi`, `mu`, `sigma` の dict、Heatmap: `heatmap_2d`, `offset_2d`, `heatmap_launch_speed`, `offset_launch_speed`, `heatmap_hit_distance`, `offset_hit_distance` の dict。D = `num_reg_targets`（デフォルト 4） |
...
CenterNet スタイルのヒートマップベース回帰。3 つのサブヘッドで構成される。
...
| `heatmap_value_range` | `[-4.0, 4.0]` | 正規化値域 |
