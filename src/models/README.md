...
models/
  ...
  components/
    ...
    backbones.py       # DNNBackbone, ResDNNBackbone, SelfAttentionBackbone（レジストリ登録）
...
#### `attention` — Self-Attention Backbone

FT-Transformer 風のバックボーン。フラット特徴量ベクトルを学習可能な線形射影でトークン列に変換し、Transformer Encoder で処理する。

<div align="center">
<table style="border-collapse: separate; border-spacing: 4px 0;">
<tr>
  <td style="background:#e8f5e9; border:2px solid #66bb6a; border-radius:8px; padding:8px 14px; text-align:center; font-size:13px;"><b>Linear</b><br><sub>→ (B, T, d)</sub></td>
  <td style="border:none; text-align:center; font-size:18px; color:#546e7a;">→</td>
  <td style="background:#e8f5e9; border:2px solid #66bb6a; border-radius:8px; padding:8px 14px; text-align:center; font-size:13px;"><b>[CLS] +<br>Pos Embed</b></td>
  <td style="border:none; text-align:center; font-size:18px; color:#546e7a;">→</td>
  <td style="background:#e8f5e9; border:2px solid #66bb6a; border-radius:8px; padding:8px 14px; text-align:center; font-size:13px;"><b>Transformer<br>Encoder</b><br><sub>× attn_num_layers</sub></td>
  <td style="border:none; text-align:center; font-size:18px; color:#546e7a;">→</td>
  <td style="background:#e8f5e9; border:2px solid #66bb6a; border-radius:8px; padding:8px 14px; text-align:center; font-size:13px;"><b>Pool</b><br><sub>CLS / mean</sub></td>
  <td style="border:none; text-align:center; font-size:18px; color:#546e7a;">→</td>
  <td style="background:#e8f5e9; border:2px solid #66bb6a; border-radius:8px; padding:8px 14px; text-align:center; font-size:13px;"><b>LN → Linear</b><br><sub>→ (B, out)</sub></td>
</tr>
</table>
</div>

- **トークン化**: `nn.Linear(input_dim, num_tokens * attn_token_dim)` で入力を `(B, T, d)` のトークン列に射影。`T = ceil(input_dim / attn_token_dim)` で自動算出
- **位置埋め込み**: 学習可能な位置埋め込みをトークンに加算
- **CLS プーリング** (`attn_pool: cls`): 先頭に `[CLS]` トークンを追加し、Encoder 出力の CLS トークンを使用
- **Mean プーリング** (`attn_pool: mean`): 全トークン出力の平均
- **出力次元**: `backbone_hidden[-1]`（他バックボーンと同様）
- **Transformer 設定**: `norm_first=True`（Pre-LN）、GELU 活性化、FFN 次元 = `attn_token_dim * attn_ff_multiplier`
...
| `backbone_type` | `"resdnn"` | `"dnn"`, `"resdnn"`, `"attention"` | Backbone の種類 |
| `backbone_hidden` | `[512, 256, 128]` | — | 各層の隠れ次元（attention の場合は最後の値が出力次元） |
| `dropout` | `0.2` | — | Dropout 率 |
| `attn_num_heads` | `4` | — | Self-Attention のヘッド数（attention のみ） |
| `attn_num_layers` | `2` | — | Transformer Encoder のレイヤー数（attention のみ） |
| `attn_token_dim` | `64` | — | トークン次元 d_model（attention のみ） |
| `attn_ff_multiplier` | `4` | — | FFN次元の倍率（attention のみ） |
| `attn_pool` | `"cls"` | `"cls"`, `"mean"` | プーリング方式（attention のみ） |
...
model:
  ...
  backbone_hidden: [128]        # 出力次元
  dropout: 0.2
  attn_num_heads: 4
  attn_num_layers: 3
  attn_token_dim: 64
  attn_ff_multiplier: 4
  attn_pool: cls                # "cls" or "mean"
  head_hidden: [64]
```
...
class MyBackbone(nn.Module):
    def __init__(self, input_dim: int, hidden_dims: list[int], dropout: float, *, cfg=None):
        super().__init__()
        ...
        self._output_dim = ...
...
`cfg` は `ModelConfig` インスタンス。backbone 固有のパラメータが必要な場合に使用する（`SelfAttentionBackbone` の `attn_*` 設定等）。不要なら `**kwargs` で無視可能。
...
| 項目 | 要件 |
|------|------|
| Backbone | `__init__(input_dim, hidden_dims, dropout, *, cfg=None)` / `output_dim` プロパティ / `forward(x) → Tensor` |
