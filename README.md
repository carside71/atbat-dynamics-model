...
- [GitHub Pages の設定](#github-pages-の設定)

## デモ

> **Prediction Viewer** — モデルの予測結果をサンプル単位でインタラクティブに確認できます。
>
> 投球コース・スイング予測・打球種別・打球速度/角度の予測確率を 1 枚のカードにまとめて表示し、キーボード操作でサンプルを切り替えられます。

<a href="https://<YOUR_GITHUB_USERNAME>.github.io/atbat-dynamics-model/viewer_ohtani.html">
  <img src="figs/viewer_preview.svg" alt="Prediction Viewer デモ" width="800">
</a>

**[▶ ライブデモを開く](https://<YOUR_GITHUB_USERNAME>.github.io/atbat-dynamics-model/viewer_ohtani.html)**

> [!NOTE]
> 上記リンクを有効にするには [GitHub Pages の設定](#github-pages-の設定) が必要です。
> リンク内の `<YOUR_GITHUB_USERNAME>` を実際の GitHub ユーザー名に置き換えてください。
...
`docs/` ディレクトリに Prediction Viewer のデモページが用意されています。GitHub Pages を有効にすると、ブラウザからインタラクティブに予測結果を確認できます。

### 手順

1. GitHub リポジトリの **Settings → Pages** を開く
2. **Source** で **Deploy from a branch** を選択
3. **Branch** を `main`（またはデフォルトブランチ）、フォルダを `/docs` に設定して **Save**
4. 数分後、`https://<username>.github.io/atbat-dynamics-model/` でデモが公開される
5. README 冒頭のデモリンク内 `<YOUR_GITHUB_USERNAME>` を実際のユーザー名に置き換える

### デモビューアの更新

新しいモデルの予測結果でデモを更新する場合:

```bash
# 1. 新しいビューア HTML を生成
python3 tools/generate_viewer.py \
  --pred-dir outputs/.../test/YYYY-MM-DD-HHMMSS \
  --batter Ohtani \
  --output docs/viewer_ohtani.html

# 2. コミット & プッシュで自動反映
git add docs/
git commit -m "Update demo viewer"
git push
```
